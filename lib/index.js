 ...
 * @param {string} options.customData
 * @param {number} options.vendorId
 * @param {string} options.productId
 * @param {string} options.serverPort
 * @returns 
 */
async function createSubscription({ browser, customData, serverPort, vendorId, productId }) {
    const page = await browser.newPage()
    const id = randomUUID()
    await page.goto(`http://127.0.0.1:${serverPort}?customData=${encodeURIComponent(customData)}&id=${id}&vendorId=${vendorId}&productId=${productId}`)
 ...
 * @param {string|object} options.customData=''
 * @returns {Promise.<CreateSubscriptionResult>}
 */
module.exports.createActivePaddleSubscription = async function ({ serverPort = 9876, vendorId, productId, customData = '' }) {
    ...
    try {
        const { checkout, order } = await createSubscription({ browser, customData, serverPort, vendorId, productId })
